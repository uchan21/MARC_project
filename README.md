...
### PRAROB edit
We recommend using Docker, but if you really want to install all of the YOLO stack natively, depending on the Ubuntu version you are using, you may receive Pip warnings about virtual environments, using Pipx, installing through apt and so on. Some of these packages are not available through apt (at least not natively). To circumvent these Pip warnings, you can use:
```shell
pip install <package>==version --break-system-packages
```
**NOTE**: this may in fact break your system packages. 
...
**Note**: make sure to set up the camera topic correctly in the launch file. If you do not have a GPU, you can also run inference on CPU, change the device in the launch file. 
...
#### PRAROB edit: How to view the detections
You can use **/yolo/debug_image** topic and _rqt_image_view_, just run:

```shell
ros2 run rqt_image_view rqt_image_view
```
and select the _debug_image_ topic in the dropdown menu.

#### PRAROB edit: How to process the detections:
You need to subscribe to the **/yolo/detections** topic (message type is **yolo_msgs/msg/DetectionArray**), consisting of a header and **detections** array. An example of subscribing and processing detection array message is provided in yolo_ros/yolo_ros/tracking_node.py:

https://github.com/larics/prarob_yolo/blob/acc12c7094eb5055eab66077a7d03c4f87c9c7c0/yolo_ros/yolo_ros/tracking_node.py#L143
...
#### PRAROB related parameters:
- **model**: YOLO model (default: yolov8m.pt)
  
You can use any of the available models. If you do not have the weights, the package will automatically download them from the Ultralytics repo. 
- **device**: GPU/CUDA (default: cuda:0)
  
If you do not have GPU, use device _cpu_
- **threshold**: detection threshold (default: 0.5)
  
The confidence of the detection. Each detection from YOLO is assigned a confidence value (larger values indicate the network is more confident that the detection belongs to a given class). You can use this threshold to make your detector more or less permissive. If you have a lot of false negatives (the network is not detecting the objects of interest) you can try lowering this threshold. If you have a lot of false positives, you can try increasing it.

- **iou**: intersection Over Union (IoU) threshold for Non-Maximum Suppression (NMS) (default: 0.7)
  
Non-Maximum Suppresion is used to eliminate overlapping bounding boxes of the same object class, so if you receive multiple detections of the same object, you can try increasing this value. 
- **imgsz_height**: image height for inference (default: 480)
- **imgsz_width**: image width for inference (default: 640)
  
Lowering resolution of the inference image will speed up the process (more obvious on the CPU). On the other hand, lower resolution means that the bounding boxes will be less accurate, which may result in errors in reconstruction of the position of the object. 
- **max_det**: maximum number of detections allowed per image (default: 300)

You probably do not need 300 detections for PRAROB scenario. 
- **input_image_topic**: camera topic of RGB images (default: /camera/rgb/image_raw)
  
**THIS IS WHERE YOU NEED TO INSERT YOUR CAMERA TOPIC**
- **use_tracking**: whether to activate tracking after detection (default: True)

You can set this to false since your objects will not be moving. 
